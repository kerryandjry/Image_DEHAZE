...
void HazeRemovalUseDarkChannelPrior(unsigned char * Src,unsigned char * Dest,unsigned char * darkchannel,int Width,int Height,int Stride, int Radius ,int GuideRadius, int MaxAtom, float Omega,float Epsilon,float T0 )
{
...
int main(int argc, char ** argv)
{
	...
	srcData = src.data;
	destData = dest.data;
	darkchannelData = darkchannel.data;

	HazeRemovalUseDarkChannelPrior(srcData, destData, darkchannelData, src.cols, src.rows, STRIDE, Blocksize, GuideBlocksize, Airlight, OMEGA * 0.01f, EPSILON * 0.001f, TAU * 0.01f);

	dest.data = destData;
	darkchannel.data = darkchannelData;

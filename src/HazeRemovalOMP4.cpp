...
float* BoxBlurGray(float* Data, int Width, int Height, int Radius)
{
	...
	for (Y = 0; Y < Height; Y++)
	{
		...
		Pointer[0] = Sum * InvAmount;
		//#pragma omp parallel for default(none) reduction(-: Sum) private(X) shared(Radius, Width, ColSum, RowOffset, Pointer, InvAmount)
		for (X = 1; X < Width; X++)
		{
			/*
			if (X == 0)
...
void HazeRemovalUseDarkChannelPrior(unsigned char * Src,unsigned char * Dest,int Width,int Height,int Stride, int Radius ,int GuideRadius, int MaxAtom, float Omega,float Epsilon,float T0 )
{
	...
	{
		ImgPt = Src + Y * Stride;
		#pragma omp parallel for reduction(+: SumB, SumG, SumR, Amount)// shared(Amount)
		for (X = 0; X < Width; X++)
		{
			/*
			if (*DarkPt >= Threshold)													
	...
	SP = Q; 
	//#pragma omp parallel for private(Transmission, Value)

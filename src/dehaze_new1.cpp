...
//#pragma omp threadprivate ( windowsMat, preCal, meanTmp, covTmp, meanMatrix1, covMatrix, meanMatrix2, Ij )
void GetLMatrix(CvSparseMat *LMat, IplImage *src)
{
	...
	CvMat *windowsMat, *Ij;
	CvMat *preCal[1];
	CvMat *meanTmp, *covTmp, *meanMatrix1, *meanMatrix2, *covMatrix;
	CvMat *mat;
	float tmp;
	int kronecker, i, j, k, m;

	#pragma omp parallel private (windowsMat, preCal, meanTmp, covTmp, meanMatrix1, meanMatrix2, covMatrix , Ij, mat, tmp, kronecker, i, j, k, m) 
	for (height = 0; height <= src->height - WNDSIZE; ++height)
	{
		#pragma omp parallel for ordered
		for (width = 0; width <= src->width - WNDSIZE; ++width)
		{
			//printf("i=%d,j=%d\n",height,width);
			//for window's 9(window * window) * 3(RGB) matrix
			//CvMat *windowsMat = cvCreateMat(WNDSIZE * WNDSIZE, 3, CV_32FC1);
			windowsMat = cvCreateMat(WNDSIZE * WNDSIZE, 3, CV_32FC1);
			...
			meanTmp = cvCreateMat(1, WNDSIZE, CV_32FC1);
			//CvMat *covTmp = cvCreateMat(WNDSIZE, WNDSIZE, CV_32FC1);
			covTmp = cvCreateMat(WNDSIZE, WNDSIZE, CV_32FC1);
			...
			meanMatrix1 = cvCreateMat(WNDSIZE * WNDSIZE, 3, CV_32FC1), meanMatrix2 = cvCreateMat(3, WNDSIZE * WNDSIZE, CV_32FC1);
			//CvMat *covMatrix = cvCreateMat(WNDSIZE, WNDSIZE, CV_32FC1);
			covMatrix = cvCreateMat(WNDSIZE, WNDSIZE, CV_32FC1);
			...
			Ij = cvCreateMat(3, WNDSIZE * WNDSIZE, CV_32FC1);
			//#pragma omp for
			for (k = 0; k < WNDSIZE; ++k)
			{
				for (m = 0; m < WNDSIZE; ++m)
			...
			mat = cvCreateMat(WNDSIZE * WNDSIZE, WNDSIZE * WNDSIZE, CV_32FC1);
			...
			for (i = 0; i < WNDSIZE; ++i)
			{
				for (j = 0; j < WNDSIZE; ++j)
				{
					for (k = 0; k < WNDSIZE; ++k)
					{
						for (m = 0; m < WNDSIZE; ++m)
						{
							kronecker = (i * WNDSIZE + j) == (k * WNDSIZE + m) ? 1 : 0;
                                               //    #pragma omp critical
                                                    {
							tmp = cvGetReal2D(LMat, (height + i) * src->width + width + j, (height + k) * src->width + width + m)
...
int main( int argc, char ** argv) 
{
	...
	for (int n = 0; n < 1; ++n)
	{
		...
		cvShowImage("Dehaze Image", dehazeImage);
                cvSaveImage("Dehazed.jpg",dehazeImage);
		if (cvWaitKey() == 27)
			break;
		for (int i = 0; i < src->height; ++i)
		{

...
int main(int argc, char ** argv)
{
	...
	IplImage *src = cvLoadImage(picname[0]);

	IplImage *dest = cvCreateImage(cvGetSize(src), src->depth, src->nChannels);

	HazeRemovalUseDarkChannelPrior(src, dest, src->width, src->height, STRIDE, Blocksize, GuideBlocksize, Airlight, OMEGA * 0.01f, EPSILON * 0.001f, TAU * 0.01f);

	t2 = get_seconds();

	cvNamedWindow("Source Image", CV_WINDOW_AUTOSIZE);
	cvShowImage("Source Image", src);
	cvNamedWindow("HazeRemoval Image", CV_WINDOW_AUTOSIZE);
	cvShowImage("HazeRemoval Image", dest); 
	//cvNamedWindow("Image", CV_WINDOW_AUTOSIZE);
	//cvShowImage("Image", );
	cvSaveImage("HazeRImage.jpg", dest);

	cvReleaseImage(&src);
	cvReleaseImage(&dest);
	cvDestroyWindow("Source Image"); 
	cvDestroyWindow("HazeRemoval Image");
	//cvDestroyWindow("");
